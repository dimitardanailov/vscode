...
export class InstallAction extends Action {
	...
	run(): TPromise<any> {
		...
		alert(localize('installExtensionStart', "Installing ${0}. An editor is now open with more details on this extension", this.extension.id));
	...
	private install(extension: IExtension): TPromise<void> {
		return this.extensionsWorkbenchService.install(extension).then(() => {
			alert(localize('installExtensionComplete', "${0} is installed. Please reload Visual Studio Code to enable it."));
		}, err => {
...
export class UninstallAction extends Action {
	...
	run(): TPromise<any> {
		alert(localize('uninstallExtensionStart', "Uninstalling ${0} started.", this.extension.id));

		return this.extensionsWorkbenchService.uninstall(this.extension).then(() => {
			alert(localize('uninstallExtensionComplete', "Please reload Visual Studio Code to complete the uninstallation of ${0}.", this.extension.id));
		});
	}
...
export class UpdateAction extends Action {
	...
	run(): TPromise<any> {
		alert(localize('updateExtensionStart', "Updating ${0} to version ${1} started.", this.extension.id, this.extension.latestVersion));
		return this.install(this.extension);
	}

	private install(extension: IExtension): TPromise<void> {
		return this.extensionsWorkbenchService.install(extension).then(() => {
			alert(localize('updateExtensionComplete', "Updating ${0} to version ${1} completed.", this.extension.id, this.extension.latestVersion));
		}, err => {
			if (!extension.gallery) {
				return this.notificationService.error(err);
			}
